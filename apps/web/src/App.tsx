...
const formatAiError = (err: unknown): string => {
  ...
  if (msg.includes("unauthorized"))
    return "Silakan masuk terlebih dahulu untuk menggunakan fitur AI.";
  if (msg.includes("insufficient_points"))
    return "Poin Anda tidak cukup untuk melakukan aksi ini.";
...
const App: React.FC = () => {
  ...
  const [pointsModalInfo, setPointsModalInfo] = useState<{
    required: number;
    remaining: number;
    action: string;
  }>({
  ...
  const [curriculumSubTab, setCurriculumSubTab] =
    useState<CurriculumSubTab>("syllabus");
  ...
  const [currentQuizQuestionIndex, setCurrentQuizQuestionIndex] =
    useState<number>(0);
  const [currentQuizModuleInfo, setCurrentQuizModuleInfo] = useState<{
    title: string;
    moduleMaterial: string;
  } | null>(null);
  const [quizAttemptCompleted, setQuizAttemptCompleted] =
    useState<boolean>(false); // True when a quiz is finished, to show summary
  const [reviewingQuiz, setReviewingQuiz] = useState<StoredQuizAttempt | null>(
    null,
  ); // Holds a historical quiz for review

  const [tutorSession, setTutorSession] = useState<TutorSession>({
    chat: null,
    history: [],
  ...
  const [loadingStepMessage, setLoadingStepMessage] = useState<string | null>(
    null,
  );
  ...
  // Exam Feature State
  const [activeExamModuleInfo, setActiveExamModuleInfo] = useState<{
    title: string;
    moduleMaterial: string;
  } | null>(null);
  const [examViewMode, setExamViewMode] =
    useState<ExamViewMode>("module_selection"); // Controls internal state of 'exam' related tabs
  const [currentExamConfigForView, setCurrentExamConfigForView] =
    useState<Partial<ExamConfiguration> | null>(null);
  const [currentExamQuestions, setCurrentExamQuestions] = useState<
    ExamQuestion[] | null
  >(null); // Active exam being taken
  const [currentExamAttempt, setCurrentExamAttempt] =
    useState<ExamAttempt | null>(null);
  const [currentExamQuestionIndex, setCurrentExamQuestionIndex] =
    useState<number>(0);
  const [examAttemptCompleted, setExamAttemptCompleted] =
    useState<boolean>(false); // True when an exam is finished
  ...
  const [examTimer, setExamTimer] = useState<{
    ...
    isActive: boolean;
  }>({ timeLeft: 0, intervalId: null, isActive: false });
  ...
  const {
    ...
    selectedMatchItemId,
    setSelectedMatchItemId,
    matchGameActive,
    setMatchGameActive,
    matchGameStartTime,
    setMatchGameStartTime,
    matchGameTimeElapsed,
    setMatchGameTimeElapsed,
  ...
  // --- Resources Tab State ---
  const [currentLearningResources, setCurrentLearningResources] =
    useState<LearningResource | null>(null);
  const [isFetchingResources, setIsFetchingResources] =
    useState<boolean>(false);
  const [fetchResourcesError, setFetchResourcesError] = useState<string | null>(
    null,
  );
  // --- End Resources Tab State ---

  // State for new LMS-style Material view
  const [selectedMaterialModuleIndex, setSelectedMaterialModuleIndex] =
  ...
  const currentCurriculumModulesMap = useMemo(() => {
    ...
    if (
      ...
      currentLearningJourneyMemo.curriculum &&
      currentLearningJourneyMemo.curriculum.modules
    ) {
    ...
    try {
      ...
      if (storedHistory) {
        ...
        const updatedHistory = parsedHistory.map((item) => {
          const newModuleCompletionStatus: Record<
            string,
            { summaryLoaded: boolean; quizTaken: boolean }
          > = {};
          ...
          item.curriculum.modules.forEach((module) => {
            ...
            if (
              !quizTaken &&
              item.quizHistory?.some((qh) => qh.moduleTitle === module.title)
            ) {
              quizTaken = true;
            }
            newModuleCompletionStatus[module.title] = {
              summaryLoaded,
              quizTaken,
            };
          });

          const updatedExamHistory = item.examHistory
            ? item.examHistory.map((eh) => ({
                ...eh,
                config: {
                  ...eh.config,
          ...
          const currNeedsUpdate = item.curriculum.modules.some(
            (m) =>
              m.isLoading ||
              (m.loadingError && m.loadingError !== null) ||
              (m.lectureSummary && !m.moduleMaterial),
          );
          return {
            ...item,
            curriculum: currNeedsUpdate
              ? {
                  ...item.curriculum,
                  modules: item.curriculum.modules.map((m) => ({
                    ...
                    moduleMaterial:
                      m.moduleMaterial || m.lectureSummary || undefined,
                  })),
                }
              : item.curriculum,
            ...
            overallProgress:
              typeof item.overallProgress === "number"
                ...
                : 0,
            journeyCompleted:
              typeof item.journeyCompleted === "boolean"
                ? item.journeyCompleted
                : false,
            quizHistory: item.quizHistory
              ? item.quizHistory.map((qh) => ({
                  ...qh,
                  quiz: qh.quiz.map((q) => ({
                    ...q,
                    userAnswer: q.userAnswer,
                    ...
                    feedbackShown:
                      q.feedbackShown !== undefined ? q.feedbackShown : true,
                  })),
                }))
              : [],
        ...
        const shouldRestore =
          ...
          (isStandardRefresh() ||
            ...
            (typeof window !== "undefined" &&
              window.location.search.includes("testRestore")));
        if (shouldRestore && checkpoint && checkpoint.selectedHistoryItemId) {
          const targetItem = updatedHistory.find(
            (item) => item.id === checkpoint.selectedHistoryItemId,
          );
          if (targetItem) {
            setTopic(targetItem.topic);
            setTargetLanguage(targetItem.targetLanguage);
            const needsReset = targetItem.curriculum.modules.some(
              (m) => m.isLoading || (m.loadingError && m.loadingError !== null),
            );
            setCurriculum(
              needsReset
                ? {
            ...
            if (langNameLower.includes("bahasa indonesia")) {
              ...
              langNameLower !== "english" &&
              targetItem.targetLanguage.trim() !== ""
            ) {
            ...
            setViewMode(
              checkpoint.viewMode && checkpoint.viewMode !== "loading"
                ? checkpoint.viewMode
                : "results",
            );
            ...
            if (
              typeof checkpoint.selectedMaterialModuleIndex === "number" &&
              checkpoint.selectedMaterialModuleIndex >= 0
            ) {
              const validIndex = Math.min(
                checkpoint.selectedMaterialModuleIndex,
                Math.max(0, targetItem.curriculum.modules.length - 1),
              );
              setSelectedMaterialModuleIndex(validIndex);
            } else {
              setSelectedMaterialModuleIndex(
                targetItem.curriculum.modules.length > 0 ? 0 : null,
              );
            }
            ...
            if (typeof checkpoint.currentFlashcardIndexInStack === "number") {
              setCurrentFlashcardIndexInStack(
                checkpoint.currentFlashcardIndexInStack,
              );
            }
            if (checkpoint.activeFlashcardModuleTitle) {
              const matchedMod = targetItem.curriculum.modules.find(
                (m) => m.title === checkpoint.activeFlashcardModuleTitle,
              );
              if (matchedMod && matchedMod.moduleMaterial) {
                setActiveFlashcardModuleInfo({
                  title: matchedMod.title,
                  moduleMaterial: matchedMod.moduleMaterial,
                });
              }
            }
            if (checkpoint.currentQuizModuleTitle) {
              const matchedMod = targetItem.curriculum.modules.find(
                (m) => m.title === checkpoint.currentQuizModuleTitle,
              );
              if (matchedMod && matchedMod.moduleMaterial) {
                setCurrentQuizModuleInfo({
                  title: matchedMod.title,
                  moduleMaterial: matchedMod.moduleMaterial,
                });
              }
            ...
            if (checkpoint.activeExamModuleTitle) {
              const matchedMod = targetItem.curriculum.modules.find(
                (m) => m.title === checkpoint.activeExamModuleTitle,
              );
              if (matchedMod && matchedMod.moduleMaterial) {
                setActiveExamModuleInfo({
                  title: matchedMod.title,
                  moduleMaterial: matchedMod.moduleMaterial,
                });
              }
  ...
  useEffect(() => {
    if (selectedHistoryItemId) {
      const currentHistItem = historyItems.find(
        (h) => h.id === selectedHistoryItemId,
      );
      if (currentHistItem) {
        if (currentHistItem.flashcardDecks) {
          ...
          if (
            activeFlashcardModuleInfo &&
            currentHistItem.flashcardDecks[activeFlashcardModuleInfo.title]
          ) {
            setCurrentFlashcardDeck(
              currentHistItem.flashcardDecks[activeFlashcardModuleInfo.title],
            );
  ...
  const calculateAndUpdateJourneyProgress = useCallback(
    (itemIdToUpdate: string) => {
      setHistoryItems((prevItems) => {
        ...
        if (
          ...
          !currentItem.sevenDayPlan ||
          !currentItem.moduleCompletionStatus
        )
          return prevItems;

        const totalModules = currentItem.curriculum.modules.length;

        const moduleStatuses = Object.values(
          currentItem.moduleCompletionStatus,
        ) as { summaryLoaded: boolean; quizTaken: boolean }[];
        const { completedModulesSummaries, completedQuizzes } =
        ...
        const completedPlanTasks = Object.values(
          currentItem.planTaskCompletionStatus,
        ).reduce((count, status) => count + (status ? 1 : 0), 0);

        const totalCompletableUnits = totalModules * 2 + totalPlanTasks;
        const completedUnits =
          completedModulesSummaries + completedQuizzes + completedPlanTasks;

        let newOverallProgress = 0;
        if (totalCompletableUnits > 0) {
          newOverallProgress = Math.min(
            100,
            (completedUnits / totalCompletableUnits) * 100,
          );
        ...
        if (newOverallProgress >= 100 && !currentItem.journeyCompleted) {
          ...
          setTimeout(
            () =>
              alert(
                `Congratulations! You've completed the learning journey for "${currentItem.topic}" and reached Level ${finalUserLevel}!`,
              ),
            100,
          );
        }
        ...
        if (
          currentItem.overallProgress === newOverallProgress &&
          (currentItem.journeyCompleted || false) ===
            (updatedItem.journeyCompleted || false)
        ) {
          return prevItems;
        }
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        "adaptivaStudyHistory",
        JSON.stringify(historyItems),
      );
      if (selectedHistoryItemId) {
        const itemExists = historyItems.some(
          (item) => item.id === selectedHistoryItemId,
        );
        if (itemExists) {
  ...
  const handleFormSubmit = useCallback(
    async (submittedTopic: string, submittedTargetLanguage: string) => {
      ...
      if ((user.points ?? 100) < 20) {
        setPointsModalInfo({
          ...
          action: "Generate Kurikulum & 7-Day Plan",
        });
      ...
      setLoadingStepMessage(
        "Analyzing topic and crafting curriculum outline...",
      );
      setError(null);
      ...
      try {
        curriculumOutlineData = await generateInitialCurriculumOutline(
          submittedTopic,
          submittedTargetLanguage,
        );
        if (!curriculumOutlineData)
          throw new Error("Failed to generate curriculum outline.");

        setLoadingStepMessage(
          "Compiling your 7-day accelerated learning plan & fetching resources...",
        );
        const moduleTitles = curriculumOutlineData.modules.map(
          (module) => module.title,
        );

        const planPromise = generateSevenDayPlan(
          submittedTopic,
          curriculumOutlineData.syllabus,
          moduleTitles,
          submittedTargetLanguage,
        );
        const resourcesPromise = fetchLearningResources(
          submittedTopic,
          submittedTargetLanguage,
        ).catch((resErr) => {
          console.warn(
            "Failed to fetch learning resources during initial setup, but continuing:",
            resErr,
          );
        ...
        const [generatedPlan, resourcesResult] = await Promise.all([
          planPromise,
          resourcesPromise,
        ]);
        ...
        if (resourcesResult === undefined) {
          ...
          setFetchResourcesError(
            "Sumber belajar tidak ditemukan untuk topik ini.",
          );
          fetchedResources = null;
        } else {
        ...
        const modulesWithLoadingErrorStatus = curriculumOutlineData.modules.map(
          (m) => ({
            ...m,
            isLoading: false,
            loadingError: null,
        ...
        const initializedCurriculum = {
          ...curriculumOutlineData,
          modules: modulesWithLoadingErrorStatus,
        };
        ...
        const initialModuleCompletionStatus: Record<
          string,
          { summaryLoaded: boolean; quizTaken: boolean }
        > = {};
        initializedCurriculum.modules.forEach((module) => {
          initialModuleCompletionStatus[module.title] = {
            summaryLoaded: false,
            quizTaken: false,
          };
        });
        ...
        setHistoryItems((prev) =>
          [
            newHistoryItem,
            ...prev.filter((item) => item.id !== newHistoryItem.id),
  ...
  const handleLoadModuleDetails = useCallback(
    async (moduleIndex: number): Promise<string | null> => {
      if (status !== "authenticated" || !user) {
        setIsAuthModalOpen(true);
        return null;
      }
      ...
      try {
        const detailData = await generateModuleLectureSummary(
          moduleToLoad.title,
          curriculum.topic,
          targetLanguage,
        );
        if (detailData && detailData.moduleMaterial) {
          setCurriculum((prev) => {
            ...
            setHistoryItems((currentHistoryItems) => {
              const itemIndex = currentHistoryItems.findIndex(
                (h) => h.id === selectedHistoryItemId,
              );
              ...
              const newModuleCompletionStatus = {
                ...
                [moduleToLoad.title]: {
                  ...(hItem.moduleCompletionStatus[moduleToLoad.title] || {
                    quizTaken: false,
                  }),
                  summaryLoaded: true,
                },
          ...
          throw new Error(
            `Failed to load details for module: ${moduleToLoad.title}`,
          );
        }
      } catch (err) {
        console.error(err);
        const errorMessage =
          formatAiError(err) ||
          `Error loading details for ${moduleToLoad.title}.`;
        setCurriculum((prev) => {
          if (!prev) return null;
          const updatedModules = [...prev.modules];
          updatedModules[moduleIndex] = {
  ...
  const handleSelectHistoryItem = useCallback(
    (id: string) => {
      ...
      if (item) {
        ...
        const needsReset = item.curriculum.modules.some(
          (m) => m.isLoading || (m.loadingError && m.loadingError !== null),
        );
        const curriculumFromHistory: Curriculum = needsReset
          ? {
              ...item.curriculum,
              modules: item.curriculum.modules.map((m) => ({
                ...m,
        ...
        setCurriculumSubTab("syllabus");
        setSelectedMaterialModuleIndex(
          item.curriculum.modules.length > 0 ? 0 : null,
        );
  ...
  const handleTogglePlanTask = useCallback(
    (dayNumber: number, totalSubtasksCount: number = 0) => {
      ...
      setHistoryItems((prevItems) => {
        ...
        const isCurrentlyCompleted =
          item.planTaskCompletionStatus[dayNumber] || false;
  ...
  const handleTogglePlanSubtask = useCallback(
    (dayNumber: number, subtaskIndex: number, totalSubtasksCount: number) => {
      if (!selectedHistoryItemId) return;
      ...
      setHistoryItems((prevItems) => {
        ...
        for (let i = 0; i < totalSubtasksCount; i++) {
          ...
          const subVal =
            subKey in updatedSubtaskStatus
  ...
  const handleGenerateQuiz = useCallback(
    async (module: CurriculumModule) => {
      ...
      if (!module.moduleMaterial) {
        setError(
          "Please load the module material first before generating a quiz.",
        );
      ...
      setCurrentQuizModuleInfo({
        title: module.title,
        moduleMaterial: module.moduleMaterial,
      });
      ...
      try {
        ...
        if (quizData && quizData.length > 0) {
          ...
          setError(
            "Failed to generate quiz questions for this module. The AI might not have found enough content or an issue occurred.",
          );
          setViewMode("results");
          setCurriculumSubTab("quiz");
        }
  ...
  const handleQuizAnswer = (questionId: string, answer: string) => {
    if (
      ...
      reviewingQuiz ||
      (quiz[currentQuizQuestionIndex] &&
        quiz[currentQuizQuestionIndex].feedbackShown)
    )
      return;

    setQuiz((prevQuiz) =>
      prevQuiz!.map((q, index) =>
        index === currentQuizQuestionIndex && q.id === questionId
          ? { ...q, userAnswer: answer }
          : q,
      ),
    );
    ...
    setHistoryItems((prevItems) => {
      ...
      if (currentQuizModuleInfo) {
        for (let i = updatedQuizHistory.length - 1; i >= 0; i--) {
          if (
            updatedQuizHistory[i].moduleTitle === currentQuizModuleInfo.title
          ) {
            lastAttemptIndex = i;
            break;
          }
      ...
      if (lastAttemptIndex !== -1) {
        ...
        const lastAttemptScorePercentage =
          lastAttempt.quiz.length > 0
            ? (lastAttempt.score / lastAttempt.quiz.length) * 100
            : 0;
        const newScorePercentage =
          newQuizAttempt.quiz.length > 0
            ? (newQuizAttempt.score / newQuizAttempt.quiz.length) * 100
            : 0;

        if (lastAttemptScorePercentage < 50 && newScorePercentage >= 50) {
          updatedQuizHistory[lastAttemptIndex] = {
            ...newQuizAttempt,
            retakeInfo: targetLanguage
              .toLowerCase()
              .includes("bahasa indonesia")
              ? "Lulus setelah percobaan sebelumnya."
              : "Passed on retake after a previous attempt.",
          };
        } else {
      ...
      newItems[itemIndex] = {
        ...item,
        quizHistory: updatedQuizHistory.sort(
  ...
  const handleLoadDetailedExplanation = useCallback(
    async (questionId: string, isExamQuestion: boolean = false) => {
      ...
      if (isExamQuestion) {
        targetQuestionSource = reviewingExam
          ? reviewingExam.questions
          : currentExamQuestions;
        contextModuleTitle =
          reviewingExam?.config.moduleTitle || activeExamModuleInfo?.title;
        ...
        if (
          ...
          const originalModule = currentCurriculumModulesMap.get(
            reviewingExam.config.moduleTitle,
          )?.module;
        ...
        contextModuleTitle =
          reviewingQuiz?.moduleTitle || currentQuizModuleInfo?.title;
      ...
      if (!targetQuestionSource || !contextModuleTitle) {
        console.warn(
          "Cannot load detailed explanation: missing context (question source, title).",
        );
        return;
      }
      ...
      if (
        ...
        if (modEntry) {
          ...
          if (loadedMaterial) {
            ...
            console.warn(
              `Failed to dynamically load material for ${contextModuleTitle} for detailed explanation.`,
            );
          }
        }
      }
      ...
      if (!contextModuleMaterialContent) {
        console.warn(
          "Still no module material after attempting load for detailed explanation.",
        );
        ...
        if (qIdx !== -1) {
          const updateFn = (prevQ: (QuizQuestion | ExamQuestion)[]) => {
            ...
            updated[qIdx] = {
              ...
              detailedExplanation:
                "Error: Module material unavailable for detailed explanation.",
            };
            return updated;
          };
          if (isExamQuestion) {
            if (reviewingExam)
              setReviewingExam((prev) =>
                prev
                  ? {
                      ...prev,
                      questions: updateFn(prev.questions) as ExamQuestion[],
                    }
                  : null,
              );
            else
              setCurrentExamQuestions((prev) =>
                prev ? (updateFn(prev) as ExamQuestion[]) : null,
              );
          } else {
            if (reviewingQuiz)
              setReviewingQuiz((prev) =>
                prev
                  ? { ...prev, quiz: updateFn(prev.quiz) as QuizQuestion[] }
                  : null,
              );
            else
              setQuiz((prev) =>
                prev ? (updateFn(prev) as QuizQuestion[]) : null,
              );
          }
        }
        return;
      ...
      const questionIndex = targetQuestionSource.findIndex(
        (q) => q.id === questionId,
      );
      ...
      if (
        ...
        !currentQuestion.detailedExplanation.startsWith("Error:")
      )
        return;
      if (currentQuestion.isDetailedExplanationLoading) return;

      const updateStateForExplanation = (
        updater: (
          prevQuestions: (QuizQuestion | ExamQuestion)[],
          ...
          if (reviewingExam)
            setReviewingExam((prev) =>
              prev
                ? {
                    ...prev,
                    questions: updater(prev.questions) as ExamQuestion[],
                  }
                : null,
            );
          else
            setCurrentExamQuestions((prev) =>
              prev ? (updater(prev) as ExamQuestion[]) : null,
            );
        } else {
          if (reviewingQuiz)
            setReviewingQuiz((prev) =>
              prev
                ? { ...prev, quiz: updater(prev.quiz) as QuizQuestion[] }
                : null,
            );
          else
            setQuiz((prev) =>
              prev ? (updater(prev) as QuizQuestion[]) : null,
            );
        }
      };
      ...
      try {
        const adaptableQuestion: QuizQuestion = {
          ...
          question:
            (currentQuestion as ExamQuestion).questionText ||
        ...
        if (explanationData && explanationData.detailedExplanation) {
          ...
          throw new Error(
            "Failed to load detailed explanation (empty response).",
          );
        }
      } catch (err) {
        ...
        updateStateForExplanation((prevQuestions) => {
          ...
          updated[questionIndex] = {
            ...updated[questionIndex],
            isDetailedExplanationLoading: false,
            detailedExplanation: `Error: Could not load more details. (${(err as Error).message})`,
          };
          return updated;
        });
      }
  ...
  const handleRetakeQuiz = useCallback(async () => {
    ...
    if (moduleIndex === -1) {
      setError(
        `Module "${moduleToRetakeTitle}" not found in current curriculum.`,
      );
      return;
    }
    ...
    if (!moduleForRetake.moduleMaterial) {
      ...
      if (!loadedMaterial) {
        setError(
          `Failed to load material for "${moduleToRetakeTitle}" before retaking quiz. Please try loading it from the 'Material' tab first.`,
        );
        setCurriculumSubTab("material");
        return;
      }
      ...
      if (currentLearningJourney) {
        reFetchedModule =
          currentCurriculumModulesMap.get(moduleToRetakeTitle)?.module;
      } else if (curriculum) {
        reFetchedModule =
          activeCurriculumModulesMap.get(moduleToRetakeTitle)?.module;
      }
    ...
    if (moduleForRetake && moduleForRetake.moduleMaterial) {
      setCurrentQuizModuleInfo({
        title: moduleForRetake.title,
        moduleMaterial: moduleForRetake.moduleMaterial,
      });
      await handleGenerateQuiz(moduleForRetake);
    } else {
      setError(
        `Material for "${moduleToRetakeTitle}" is still missing. Cannot generate quiz.`,
      );
      setCurriculumSubTab("material");
    }
  }, [
  ...
  const handleStartRetakeQuizFromHistoryList = useCallback(
    async (moduleTitleToRetake: string) => {
      let currentLearningJourney = currentLearningJourneyMemo;
      if (!currentLearningJourney || !selectedHistoryItemId || !curriculum) {
        setError(
          "Cannot retake quiz. Current learning session data is incomplete.",
        );
        return;
      }
      setViewMode("loading");
      ...
      if (moduleIndex === -1) {
        setError(
          `Module "${moduleTitleToRetake}" not found in the current curriculum.`,
        );
        setViewMode("results");
        return;
      }
      ...
      if (!material) {
        ...
        if (!material) {
          setError(
            `Failed to load material for "${moduleTitleToRetake}" to retake the quiz. Please try loading it from the 'Material' tab first.`,
          );
          setViewMode("results");
          setCurriculumSubTab("material");
          return;
        ...
        if (currentLearningJourney) {
          reFetchedModule =
            currentCurriculumModulesMap.get(moduleTitleToRetake)?.module;
        } else if (curriculum) {
          reFetchedModule =
            activeCurriculumModulesMap.get(moduleTitleToRetake)?.module;
        }
      ...
      if (!material) {
        setError(
          `Material for "${moduleTitleToRetake}" is still missing after attempting to load. Cannot generate quiz.`,
        );
        setViewMode("results");
        setCurriculumSubTab("material");
        return;
  ...
  const handleTutorSendMessage = useCallback(async () => {
    ...
    setTutorSession((prev) => ({
      ...
      history: [
        ...prev.history,
        { id: aiResponseId, sender: "ai", text: "...", timestamp: new Date() },
      ],
    }));

    try {
      await sendMessageToTutorStream(
        tutorSession.chat,
        userMessage.text,
        (chunkText) => {
  ...
  const handleReviewQuizFromHistory = useCallback(
    (quizRecord: StoredQuizAttempt) => {
      setReviewingQuiz(quizRecord);
      setCurrentQuizQuestionIndex(0);
      let currentLearningJourney = currentLearningJourneyMemo;
      const originalModule = currentCurriculumModulesMap.get(
        quizRecord.moduleTitle,
      )?.module;

      if (originalModule && originalModule.moduleMaterial) {
        setCurrentQuizModuleInfo({
          title: quizRecord.moduleTitle,
          moduleMaterial: originalModule.moduleMaterial,
        });
      } else {
        setCurrentQuizModuleInfo({
          title: quizRecord.moduleTitle,
  ...
  const handleInitiateExamConfig = useCallback(
    async (
      ...
      if (!material) {
        setError(
          `Module material for "${module.title}" must be loaded first. Please go to the 'Material' tab and load it.`,
        );
        setCurriculumSubTab("material");
        if (curriculum) {
          const modEntry = activeCurriculumModulesMap.get(module.title);
          if (modEntry) {
            ...
            setCurriculum((prev) => {
              ...
              updatedModules[modIdx] = {
                ...updatedModules[modIdx],
                loadingError: "Material must be loaded for exam generation.",
              };
              return { ...prev, modules: updatedModules };
            });
          }
  ...
  const handleStartRetakeExamFromHistory = useCallback(
    async (examAttemptToRetake: ExamAttempt) => {
      let currentLearningJourney = currentLearningJourneyMemo;
      if (!currentLearningJourney || !selectedHistoryItemId) {
        setError(
          "Cannot retake exam. Current learning session data is incomplete.",
        );
      ...
      await handleInitiateExamConfig(
        moduleForRetake,
        examAttemptToRetake.config,
      );
    },
    [
      selectedHistoryItemId,
      handleInitiateExamConfig,
      setError,
      currentLearningJourneyMemo,
      currentCurriculumModulesMap,
    ],
  );
    ...
    (autoSubmitted = false) => {
      ...
      setHistoryItems((prevItems) => {
        ...
        newItems[itemIndex] = {
          ...prevItems[itemIndex],
          examHistory: [
            ...(prevItems[itemIndex].examHistory || []),
            finalAttempt,
          ].sort((a, b) => b.timestamp - a.timestamp),
        };
        return newItems;
  ...
  const handleGenerateAndStartExam = useCallback(
    async (config: ExamConfiguration) => {
      if (status !== "authenticated" || !user) {
        setIsAuthModalOpen(true);
        return;
      }
      ...
      try {
        ...
        if (questions && questions.length > 0) {
          const initializedQuestions = questions.map((q) => ({
            ...
            scoreAwarded: 0,
            feedbackShown: false,
            type: "multiple-choice" as ExamQuestionType,
          }));
          ...
          const durationSeconds =
            config.timeLimitEnabled && config.durationMinutes
          ...
          throw new Error(
            "Failed to generate exam questions or no questions were returned.",
          );
        }
        setViewMode("results");
      } catch (err) {
  ...
  const handleExamAnswer = (questionId: string, answer: string) => {
    ...
    setCurrentExamQuestions((prevQuestions) =>
      prevQuestions!.map((q, index) =>
        index === currentExamQuestionIndex && q.id === questionId
          ? { ...q, userAnswer: answer }
          : q,
      ),
    );
  ...
  const handleExamNavigation = (direction: "next" | "prev") => {
    const questionsSource = reviewingExam
      ? reviewingExam.questions
      : currentExamQuestions;
    ...
    if (
      direction === "next" &&
      currentExamQuestionIndex < questionsSource.length - 1
    ) {
  ...
  const handleReviewExamFromHistory = useCallback(
    (examRecord: ExamAttempt) => {
      ...
      let currentLearningJourney = currentLearningJourneyMemo;
      const originalModule = currentCurriculumModulesMap.get(
        examRecord.config.moduleTitle,
      )?.module;
      if (originalModule && originalModule.moduleMaterial) {
        setActiveExamModuleInfo({
          title: examRecord.config.moduleTitle,
          moduleMaterial: originalModule.moduleMaterial,
        });
      } else {
        setActiveExamModuleInfo({
        ...
        console.warn(
          `Material for reviewed exam module "${examRecord.config.moduleTitle}" not readily available.`,
        );
      }
      setCurrentExamQuestions(null);
      setCurrentExamAttempt(null);
  ...
  const handleInitiateFlashcardGeneration = useCallback(
    async (module: CurriculumModule) => {
      setViewMode("loading");
      setError(null);
      let material: string | null | undefined = module.moduleMaterial;
          ...
          if (modEntry) {
            ...
            setCurriculum((prev) => {
              ...
              updatedModules[modIdx] = {
                ...updatedModules[modIdx],
                loadingError:
                  "Material must be loaded for flashcard generation.",
              };
              return { ...prev, modules: updatedModules };
            });
          }
  ...
  const quizToShowInResultOrReview = useMemo(() => {
    const quizModuleTitleForDisplay =
      currentQuizModuleInfo?.title || reviewingQuiz?.moduleTitle;
    return (
      reviewingQuiz ||
      (quizAttemptCompleted && quiz
        ? ({
            quiz: quiz,
  ...
  const handleFetchResources = useCallback(async () => {
    ...
    try {
      ...
      if (resources) {
        ...
        setFetchResourcesError(
          "Tidak dapat menemukan sumber belajar untuk topik ini.",
        );
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (
      ...
      selectedMaterialModuleIndex === null
    ) {
      setSelectedMaterialModuleIndex(0);
      const firstModule = curriculum.modules[0];
      if (
        !firstModule.moduleMaterial &&
        !firstModule.isLoading &&
        !firstModule.loadingError
      ) {
  ...
  const renderMainContent = () => {
    const largeTextBase =
      "text-lg md:text-xl text-brand-black dark:text-gray-200";
    const heading1Size =
      "text-2xl sm:text-3xl md:text-3xl font-bold text-brand-blue dark:text-blue-300";
    const heading2Size =
      "text-xl sm:text-2xl md:text-2xl font-semibold text-brand-blue dark:text-blue-300";
    ...
    const mainTabs: {
    ...
    }[] = [
      {
        name: "curriculum",
        label: "Curriculum",
        icon: <Icons.BookOpen className="w-4 h-4 md:w-5 md:h-5" />,
      },
      {
        name: "plan",
        label: "7-Day Plan",
        icon: <Icons.CalendarDays className="w-4 h-4 md:w-5 md:h-5" />,
      },
      {
        name: "resources",
        label: "Resources",
        icon: <Icons.LightBulb className="w-4 h-4 md:w-5 md:h-5" />,
      },
      {
        name: "tutor",
        label: "Tutor",
        icon: <Icons.ChatBubble className="w-4 h-4 md:w-5 md:h-5" />,
      },
    ];
    ...
    switch (viewMode) {
      ...
      case "loading":
        ...
        if (!loadingStepMessage) {
          if (isGeneratingExam) {
            ...
            curriculum?.modules.some((m) => m.isLoading) &&
            selectedMaterialModuleIndex !== null &&
            curriculum.modules[selectedMaterialModuleIndex]?.isLoading
          ) {
            const loadingModule =
              curriculum.modules[selectedMaterialModuleIndex];
            determinedLoadingMessage = `Loading material for ${loadingModule?.title || "module"}...`;
          } else if (
            quiz?.some((q) => q.isDetailedExplanationLoading) ||
            reviewingQuiz?.quiz.some((q) => q.isDetailedExplanationLoading)
          ) {
            determinedLoadingMessage = `Fetching more details for quiz question...`;
          } else if (
            currentExamQuestions?.some((q) => q.isDetailedExplanationLoading) ||
            reviewingExam?.questions.some((q) => q.isDetailedExplanationLoading)
          ) {
          ...
          } else if (isFetchingResources && activeTab === "resources") {
            // Show resource fetching message only if on resources tab or initial load
        ...
        return (
          <div className="text-center p-4 md:p-8 bg-brand-white dark:bg-brand-black rounded-lg shadow-md">
            <div className="flex justify-center mb-4">
              {" "}
              <Icons.XCircle className="w-10 h-10 md:w-12 md:h-12 text-brand-red" />
            </div>
            <h2
              className={`text-xl md:text-2xl font-semibold text-brand-red my-3`}
            >
              Oops! Something went wrong.
            </h2>
            <p className={`${largeTextBase} mb-6`}>
      ...
      case "results":
        if (!curriculum && !sevenDayPlan && !currentHistoryJourney) {
          return (
            <div className="text-center p-4 md:p-8 dark:text-gray-400">
              <p
                className={`${largeTextBase} text-brand-black/70 dark:text-gray-400`}
              >
                No content to display. Please start a new session or select one
                from history.
              </p>
            </div>
          );
        ...
        const quizModuleTitleForDisplay =
          currentQuizModuleInfo?.title || reviewingQuiz?.moduleTitle;
        const currentActiveQuizQuestionData = activeQuizForDisplay
          ? activeQuizForDisplay[currentQuizQuestionIndex]
          : null;

        const currentQuizQuestionForReviewView = quizToShowInResultOrReview
          ? quizToShowInResultOrReview.quiz[currentQuizQuestionIndex]
          : null;

        const examToShowInResultView =
          reviewingExam || (examAttemptCompleted ? currentExamAttempt : null);

        let examPercentageScore = 0;
        if (examToShowInResultView) {
          examPercentageScore =
            examToShowInResultView.maxScore > 0
              ? Math.round(
                  (examToShowInResultView.totalScore /
                    examToShowInResultView.maxScore) *
                    100,
                )
              : 0;
        }
        const examModuleTitleForTakingDisplay =
          activeExamModuleInfo?.title ||
          currentExamAttempt?.config.moduleTitle ||
          reviewingExam?.config.moduleTitle;

        const displayedModuleForMaterialView =
          curriculum && selectedMaterialModuleIndex !== null
            ? curriculum.modules[selectedMaterialModuleIndex]
            : null;
          ...
          <div className="w-full p-2 sm:p-4 md:p-6 bg-brand-white dark:bg-brand-black rounded-lg shadow-md">
            <>
              <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 pb-4 border-b border-brand-mediumGray dark:border-gray-700">
                <div
                  className={`w-full sm:w-auto mb-2 sm:mb-0 text-center ${activeTab === "curriculum" && curriculumSubTab === "material" ? "sm:text-left" : "sm:text-center"}`}
                >
                  <h1
                    className={`text-2xl sm:text-3xl md:text-4xl font-bold text-brand-blue dark:text-blue-300 truncate`}
                  >
                    Learning:{" "}
                ...
                {targetLanguage && (
                  <div
                    className={`text-sm md:text-md text-brand-black/70 dark:text-gray-300 sm:ml-4 text-center sm:text-left`}
                  >
                    Language:{" "}
              ...
              {currentHistoryJourney &&
                typeof currentHistoryJourney.overallProgress === "number" && (
                  <div className="my-4 p-3 bg-brand-lightGray dark:bg-gray-700 rounded-lg shadow border border-brand-mediumGray dark:border-gray-600">
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-brand-blue dark:text-blue-300 font-semibold text-md md:text-lg">
                        Learning Progress:
                      </span>
                      <span className="text-brand-green dark:text-green-400 font-bold text-md md:text-lg">
                        {Math.round(currentHistoryJourney.overallProgress)}%
                      </span>
                    </div>
                    <div className="w-full bg-brand-mediumGray dark:bg-gray-600 rounded-full h-2.5 md:h-3.5">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, Math.max(0, currentHistoryJourney.overallProgress))}%`,
                        }}
                        aria-valuenow={currentHistoryJourney.overallProgress}
                        aria-valuemin={0}
                        aria-valuemax={100}
                      ></div>
                    </div>
              ...
              {error &&
                (activeTab === "curriculum" ||
                  (activeTab === "resources" && fetchResourcesError)) && (
            ...
            <div className="mb-4 md:mb-6 border-b border-brand-mediumGray dark:border-gray-700">
              <nav
                className="flex space-x-1 sm:space-x-2 -mb-px overflow-x-auto scrollbar-hide"
                aria-label="Tabs"
              >
            ...
            {activeTab === "curriculum" && curriculum && (
              <div>
                    ...
                    {curriculumSubTabsList.map((subTabInfo) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (
                            subTabInfo.name === "quiz" &&
                            !reviewingQuiz &&
                            !activeQuizForDisplay
                          ) {
                            resetQuizState();
                          }
                ...
                {curriculumSubTab === "syllabus" && (
                  <Accordion
                    title={
                      <span className="font-semibold text-md md:text-lg text-brand-blue dark:text-blue-300">
                        Overall Syllabus
                      </span>
                    }
                    startOpen={true}
                  >
                    <MemoizedMarkdownRenderer
                      content={curriculum.syllabus}
                      baseTextSize={largeTextBase}
                    />
                  </Accordion>
                )}
                  ...
                  <div className="flex flex-col md:flex-row gap-4 md:gap-6 items-start">
                    ...
                    <div className="w-full md:w-2/3 lg:w-3/4 bg-brand-white dark:bg-brand-black p-3 md:p-5 rounded-lg shadow-lg border border-brand-mediumGray dark:border-gray-700 h-auto min-h-[250px] max-h-[calc(100vh-220px)] overflow-y-auto scrollbar-thin scrollbar-thumb-brand-mediumGray dark:scrollbar-thumb-gray-600">
                      {displayedModuleForMaterialView ? (
                        <>
                          <h2 className="text-xl sm:text-2xl md:text-2xl font-bold text-brand-orange dark:text-orange-400 mb-3 md:mb-4">
                            {cleanModuleTitle(
                              displayedModuleForMaterialView.title,
                            )}
                          </h2>
                          {displayedModuleForMaterialView.isLoading && (
                            <LoadingSpinner
                              message={`Loading material for ${cleanModuleTitle(displayedModuleForMaterialView.title)}...`}
                            />
                          )}
                          {!displayedModuleForMaterialView.isLoading &&
                            displayedModuleForMaterialView.moduleMaterial && (
                              <>
                                <MemoizedMarkdownRenderer
                                  content={
                                    displayedModuleForMaterialView.moduleMaterial
                                ...
                                <div className="mt-4 md:mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 border-t border-brand-mediumGray dark:border-gray-600 pt-4">
                                  ...
                                  <button
                                    onClick={() =>
                                      handleInitiateFlashcardGeneration(
                                        displayedModuleForMaterialView,
                                      )
                                    }
                                    disabled={
                          ...
                          {!displayedModuleForMaterialView.isLoading &&
                            !displayedModuleForMaterialView.moduleMaterial &&
                            !displayedModuleForMaterialView.loadingError && (
                              <button
                                onClick={() =>
                                  handleLoadModuleDetails(
                                    selectedMaterialModuleIndex!,
                          ...
                          {!displayedModuleForMaterialView.isLoading &&
                            displayedModuleForMaterialView.loadingError && (
                              <div className="mt-2">
                                <p
                                  className={`text-brand-red dark:text-red-400 text-sm md:text-md mb-2 md:mb-3`}
                                >
                        ...
                        <p
                          className={`${largeTextBase} text-brand-black/70 dark:text-gray-400 text-center py-10`}
                        >
                          Select a module from the right to view its material.
                        </p>
                      )}
                    </div>
                    ...
                    <div className="w-full md:w-1/3 lg:w-1/4 flex-shrink-0 bg-brand-lightGray dark:bg-gray-800 p-3 md:p-4 rounded-lg shadow border border-brand-mediumGray dark:border-gray-700 h-auto max-h-[calc(100vh-220px)] self-start flex flex-col overflow-y-auto scrollbar-thin scrollbar-thumb-brand-mediumGray dark:scrollbar-thumb-gray-600">
                      <div className="flex justify-between items-center mb-3 border-b border-brand-mediumGray dark:border-gray-600 pb-2">
                        <h3 className="text-lg md:text-xl font-semibold text-brand-blue dark:text-blue-300">
                          Modul
                        </h3>
                        <span className="text-xs font-semibold px-2 py-0.5 rounded bg-brand-blue/10 text-brand-blue dark:bg-blue-900/40 dark:text-blue-300">
                          {curriculum.modules.reduce(
                            (count, m) =>
                              count +
                              (currentHistoryJourney?.moduleCompletionStatus[
                                m.title
                              ]?.summaryLoaded
                                ? 1
                                : 0),
                          ...
                          /{curriculum.modules.length} Selesai
                        </span>
                      </div>
                      {curriculum.modules.length === 0 ? (
                        <p className="text-sm text-brand-black/70 dark:text-gray-400">
                          No modules for this topic.
                        </p>
                      ) : (
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="flex-shrink-0 ml-1">
                                  {module.isLoading ? (
                                    <Icons.LoadingAnimatedIcon className="w-4 h-4 text-current animate-spin" />
                                  ) : isCompleted ? (
                                    <Icons.CheckCircle className="w-4 h-4 text-brand-green" />
                                  ) : module.loadingError ? (
                                    <Icons.XCircle className="w-4 h-4 text-brand-red" />
                                  ) : (
                ...
                {curriculumSubTab === "quiz" && (
                  <div>
                    ...
                    !reviewingQuiz ? (
                      <div className="bg-brand-white dark:bg-brand-black p-4 md:p-6 rounded-lg shadow-xl border border-brand-mediumGray dark:border-gray-700">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-3 md:mb-4">
                          <h3
                            className={`${heading2Size} font-semibold text-brand-blue dark:text-blue-300 flex items-center mb-1 sm:mb-0`}
                          >
                            Quiz: {quizModuleTitleForDisplay}
                          </h3>
                          <p
                            className={`text-md md:text-lg font-semibold text-brand-black/80 dark:text-gray-300`}
                          >
                            Question {currentQuizQuestionIndex + 1} of{" "}
                            {activeQuizForDisplay.length}
                          </p>
                        </div>
                        ...
                        <div className="mb-4 md:mb-6 p-3 md:p-4 border border-brand-mediumGray dark:border-gray-600 rounded-md bg-brand-lightGray/50 dark:bg-gray-700/50">
                          <p
                            className={`font-medium text-brand-black dark:text-gray-100 mb-2 md:mb-3 ${largeTextBase}`}
                          >
                            {currentActiveQuizQuestionData.question}
                          </p>
                          <div className="space-y-1.5 md:space-y-2">
                            {currentActiveQuizQuestionData.options.map(
                              (option, i) => (
                                <label
                                  key={i}
                                  className={`flex items-center p-2.5 md:p-3 bg-brand-white dark:bg-gray-700 hover:bg-brand-mediumGray dark:hover:bg-gray-600 rounded-md cursor-pointer transition-colors border border-brand-mediumGray dark:border-gray-600 ${largeTextBase} text-sm md:text-base`}
                                >
                                  <input
                                    type="radio"
                                    ...
                                    value={option}
                                    checked={
                                      currentActiveQuizQuestionData.userAnswer ===
                                      option
                                    }
                                    onChange={() =>
                                      handleQuizAnswer(
                                        currentActiveQuizQuestionData.id,
                                        option,
                                      )
                                    }
                                    disabled={
                                  ...
                                  />
                                  <span className="ml-2 md:ml-3 text-brand-black dark:text-gray-200">
                                    {option}
                                  </span>
                                </label>
                              ),
                          ...
                          {currentActiveQuizQuestionData.feedbackShown && (
                            <div
                              className={`mt-3 md:mt-4 pt-2 md:pt-3 border-t border-brand-mediumGray dark:border-gray-600 ${currentActiveQuizQuestionData.isCorrect ? "text-brand-green dark:text-green-400" : "text-brand-red dark:text-red-400"}`}
                            >
                              <p className={`text-md md:text-lg font-semibold`}>
                                ...
                                {currentActiveQuizQuestionData.userAnswer ||
                                  "Not answered"}
                                " was{" "}
                                {currentActiveQuizQuestionData.isCorrect
                                  ? "Correct!"
                                  : "Incorrect."}
                              </p>
                              {!currentActiveQuizQuestionData.isCorrect && (
                                <p
                                  className={`text-md md:text-lg text-brand-black/90 dark:text-gray-300`}
                                >
                                  Correct answer:{" "}
                                  <span className="font-semibold text-brand-green dark:text-green-400">
                                    {
                                      currentActiveQuizQuestionData.correctAnswer
                                    }
                                  </span>
                                </p>
                              )}
                              {currentActiveQuizQuestionData.explanation && (
                                <div className="mt-2">
                                  <h5
                                    className={`text-lg md:text-xl font-semibold text-brand-blue dark:text-blue-400 mb-1`}
                                  >
                                    Explanation:
                                  </h5>
                                  <MemoizedMarkdownRenderer
                                ...
                                {currentActiveQuizQuestionData.isDetailedExplanationLoading ? (
                                  ...
                                  <>
                                    <Accordion
                                      ...
                                      title={
                                        <span className="text-lg md:text-xl font-semibold text-brand-orange dark:text-orange-400">
                                      ...
                                      startOpen={
                                        !!currentActiveQuizQuestionData.detailedExplanation &&
                                        !currentActiveQuizQuestionData.isDetailedExplanationLoading &&
                                        !currentActiveQuizQuestionData.detailedExplanation.startsWith(
                                          "Error:",
                                        )
                                      }
                                    >
                                    ...
                                    {currentActiveQuizQuestionData.detailedExplanation.includes(
                                      "Failed to load detailed explanation (empty response).",
                                    ) && (
                                      <button
                                        onClick={() =>
                                          handleLoadDetailedExplanation(
                                            currentActiveQuizQuestionData.id,
                                            false,
                                          )
                                        }
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-center mt-6 md:mt-8 space-y-2 sm:space-y-0 sm:space-x-2">
                          ...
                          {!currentActiveQuizQuestionData.feedbackShown && (
                            <button
                              onClick={handleCheckCurrentQuestionAnswer}
                              disabled={
                                !currentActiveQuizQuestionData.userAnswer
                              }
                              className={`w-full sm:w-auto px-4 py-2 md:px-6 md:py-3 bg-brand-green hover:bg-[#25632B] dark:hover:bg-green-700 text-brand-white font-semibold rounded-lg disabled:opacity-50 ${largeTextBase} text-sm md:text-base`}
                            >
                              Check Answer
                            </button>
                          )}
                          {currentActiveQuizQuestionData.feedbackShown &&
                            currentQuizQuestionIndex <
                              activeQuizForDisplay.length - 1 && (
                              <button
                                onClick={() => handleQuizNavigation("next")}
                                className={`w-full sm:w-auto px-4 py-2 md:px-6 md:py-3 bg-brand-blue hover:bg-[#004175] dark:hover:bg-blue-700 text-brand-white font-semibold rounded-lg ${largeTextBase} text-sm md:text-base`}
                              >
                                Next Question
                              </button>
                            )}
                          {currentActiveQuizQuestionData.feedbackShown &&
                            currentQuizQuestionIndex ===
                              activeQuizForDisplay.length - 1 && (
                              <button
                                onClick={handleViewQuizSummary}
                                className={`w-full sm:w-auto px-4 py-2 md:px-6 md:py-3 bg-brand-green hover:bg-[#25632B] dark:hover:bg-green-700 text-brand-white font-semibold rounded-lg ${largeTextBase} text-sm md:text-base`}
                              >
                                View Summary
                              </button>
                            )}
                    ...
                    ) : quizToShowInResultOrReview &&
                      currentQuizQuestionForReviewView ? (
                      <div className="bg-brand-white dark:bg-brand-black p-4 md:p-6 rounded-lg shadow-xl border border-brand-mediumGray dark:border-gray-700">
                        <h3
                          className={`${heading2Size} font-semibold text-brand-blue dark:text-blue-300 mb-3 md:mb-4`}
                        >
                          Quiz Result: {quizToShowInResultOrReview.moduleTitle}
                        </h3>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-4 mb-4 md:mb-6">
                          <div className="p-3 md:p-4 bg-brand-lightGray dark:bg-gray-700 rounded-lg text-center">
                            <p className="text-2xl md:text-4xl font-bold text-brand-blue dark:text-blue-400">
                              {quizToShowInResultOrReview.score}/
                              {quizToShowInResultOrReview.quiz.length}
                            </p>
                            <p
                              className={`${largeTextBase} text-brand-black/80 dark:text-gray-300 text-sm md:text-base`}
                            >
                              Score
                            </p>
                          </div>
                          <div className="p-3 md:p-4 bg-brand-lightGray dark:bg-gray-700 rounded-lg text-center">
                            <p className="text-2xl md:text-4xl font-bold text-brand-green dark:text-green-400">
                              {quizToShowInResultOrReview.quiz.length > 0
                                ? Math.round(
                                    (quizToShowInResultOrReview.score /
                                      quizToShowInResultOrReview.quiz.length) *
                                      100,
                        ...
                        <div className="mb-4 md:mb-6">
                          <h4 className="text-xl md:text-2xl font-semibold text-brand-orange dark:text-orange-400 mb-1 md:mb-2">
                          ...
                          <div className="flex h-6 md:h-8 w-full bg-brand-red/30 dark:bg-red-700/50 rounded-md overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${quizToShowInResultOrReview.quiz.length > 0 ? Math.round((quizToShowInResultOrReview.score / quizToShowInResultOrReview.quiz.length) * 100) : 0}%`,
                              }}
                        ...
                        <h4 className="text-xl md:text-2xl font-semibold text-brand-orange dark:text-orange-400 mb-2 md:mb-3">
                          Review Your Answers:
                        </h4>
                        {quizToShowInResultOrReview.quiz.map((q, index) => (
                          <Accordion
                            ...
                            title={
                              <div
                                className={`flex items-center w-full ${q.isCorrect ? "text-brand-green dark:text-green-400" : "text-brand-red dark:text-red-400"}`}
                              >
                                {q.isCorrect ? (
                                  <Icons.CheckCircle className="w-4 h-4 md:w-5 md:h-5 mr-2" />
                                ) : (
                                  <Icons.XCircle className="w-4 h-4 md:w-5 md:h-5 mr-2" />
                                )}
                          ...
                          >
                            <p
                              className={`${largeTextBase} mb-1 text-sm md:text-base`}
                            >
                              <strong>Your Answer:</strong>{" "}
                              {q.userAnswer || "Not answered"}
                            </p>
                            <p
                              className={`${largeTextBase} mb-2 text-sm md:text-base`}
                            >
                              <strong>Correct Answer:</strong> {q.correctAnswer}
                            </p>
                            <h5
                              className={`text-lg md:text-xl font-semibold text-brand-blue dark:text-blue-400 mt-2 mb-1`}
                            >
                              Explanation:
                            </h5>
                            <MemoizedMarkdownRenderer
                            ...
                            <div className="mt-2 md:mt-3">
                              {q.isDetailedExplanationLoading ? (
                                ...
                                <>
                                  <Accordion
                                    ...
                                    startOpen={
                                      !!q.detailedExplanation &&
                                      !q.isDetailedExplanationLoading &&
                                      !q.detailedExplanation.startsWith(
                                        "Error:",
                                      )
                                    }
                                  >
                                    <MemoizedMarkdownRenderer
                                      content={q.detailedExplanation}
                                      baseTextSize="text-sm md:text-lg"
                                    />
                                  </Accordion>
                                  {q.detailedExplanation.includes(
                                    "Failed to load detailed explanation (empty response).",
                                  ) && (
                                    <button
                                      onClick={() =>
                                ...
                                <button
                                  onClick={() =>
                                    handleLoadDetailedExplanation(q.id, false)
                                  }
                                  className={`px-3 py-1.5 md:px-4 md:py-2 bg-brand-orange hover:bg-[#D84315] dark:hover:bg-orange-600 text-brand-white text-sm md:text-md font-semibold rounded-lg`}
                                >
                                  Load More Details
                      ...
                      !error && (
                        <div className="text-center p-6 bg-brand-lightGray dark:bg-gray-800 rounded-lg shadow">
                          ...
                          <h3
                            className={`${heading2Size} font-semibold text-brand-blue dark:text-blue-300 mb-3`}
                          >
                            Quiz Section
                          </h3>
                          <p
                            className={`${largeTextBase} text-brand-black/80 dark:text-gray-300 mb-4`}
                          >
                            To take a quiz, please select a module from the{" "}
                            <button
                            ...
                            tab and click "Take Quiz", or select a quiz from{" "}
                            <button
                          ...
                          <p
                            className={`${largeTextBase} text-brand-black/70 dark:text-gray-400`}
                          >
                            Ensure the module material is loaded before
                            generating a quiz.
                          </p>
                        </div>
                      )
                    )}
                    {error && curriculumSubTab === "quiz" && (
                      <p
                        className={`mt-4 text-brand-red dark:text-red-400 text-center`}
                      >
                        {error}
                      </p>
                    )}
                ...
                {curriculumSubTab === "study_log" && (
                  <div className="space-y-3 md:space-y-4">
                    <h3
                      className={`${heading2Size} font-semibold text-brand-blue dark:text-blue-300 mt-4 md:mt-6 mb-2 md:mb-3`}
                    >
                      Study Log for {topic}
                    </h3>
                    ...
                    <Accordion
                      ...
                      startOpen={true}
                    >
                      {!currentHistoryJourney?.quizHistory ||
                      currentHistoryJourney.quizHistory.length === 0 ? (
                        <p
                          className={`${largeTextBase} text-brand-black/70 dark:text-gray-400`}
                        >
                          No quizzes taken for this learning journey yet.
                        </p>
                      ) : (
                        currentHistoryJourney.quizHistory.map((qh, index) => {
                          const historyPercentageScore =
                            qh.quiz.length > 0
                              ? Math.round((qh.score / qh.quiz.length) * 100)
                              : 0;
                          return (
                            <div
                              key={`${qh.moduleId}-${qh.timestamp}-${index}`}
                              className="p-3 mb-2 bg-brand-white dark:bg-gray-800 rounded-md border border-brand-mediumGray dark:border-gray-700"
                            >
                              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full">
                                <span className="font-semibold text-md text-brand-blue dark:text-blue-300 mb-1 sm:mb-0 truncate max-w-[70%] sm:max-w-none">
                                  {qh.moduleTitle}
                                </span>
                                <span className="text-xs sm:text-sm font-normal text-brand-black/80 dark:text-gray-400">
                                  Score: {qh.score}/{qh.quiz.length} (
                                  {historyPercentageScore}%) - Taken:{" "}
                                  {new Date(qh.timestamp).toLocaleDateString()}
                                  {qh.retakeInfo && (
                                    <span className="ml-2 text-brand-green dark:text-green-300 italic">
                                      ({qh.retakeInfo})
                                    </span>
                                  )}
                                </span>
                              ...
                              <div className="mt-2 flex flex-wrap gap-2 items-center">
                                <button
                                  onClick={() =>
                                    handleReviewQuizFromHistory(qh)
                                  }
                                ...
                                {historyPercentageScore < 50 && (
                                  <button
                                    onClick={() =>
                                      handleStartRetakeQuizFromHistoryList(
                                        qh.moduleTitle,
                                      )
                                    }
                    ...
                    <Accordion
                      ...
                      startOpen={true}
                    >
                      {!currentHistoryJourney?.examHistory ||
                      currentHistoryJourney.examHistory.length === 0 ? (
                        <p
                          className={`${largeTextBase} text-brand-black/70 dark:text-gray-400`}
                        >
                          No exams taken for this learning journey yet.
                        </p>
                      ) : (
                        currentHistoryJourney.examHistory.map((attempt) => {
                          const attemptScorePercentage =
                            attempt.maxScore > 0
                              ? Math.round(
                          ...
                          return (
                            ...
                            >
                              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full">
                                <span className="font-semibold text-md text-brand-blue dark:text-blue-300 truncate max-w-[70%] sm:max-w-none">
                                  {attempt.config.moduleTitle} (Exam)
                                </span>
                                <span className="text-xs sm:text-sm font-normal text-brand-black/80 dark:text-gray-400">
                                  Score: {attempt.totalScore}/{attempt.maxScore}{" "}
                                  ({attemptScorePercentage}%) - Taken:{" "}
                                  {new Date(
                                    attempt.timestamp,
                                  ).toLocaleDateString()}
                                </span>
                              ...
                              <div className="mt-2 flex flex-wrap gap-2 items-center">
                                <button
                                  onClick={() =>
                                    handleReviewExamFromHistory(attempt)
                                  }
                                  className={`px-3 py-1.5 bg-brand-blue hover:bg-[#004175] dark:hover:bg-blue-700 text-brand-white text-sm font-semibold rounded-lg`}
                                >
                                  Review This Exam
                                ...
                                {attemptScorePercentage < 50 && (
                                  <button
                                    onClick={() =>
                                      handleStartRetakeExamFromHistory(attempt)
                                    }
                                    className={`px-3 py-1.5 bg-brand-orange hover:bg-[#D84315] dark:hover:bg-orange-600 text-brand-white text-sm font-semibold rounded-lg`}
                                  >
                    ...
                    <Accordion
                      ...
                      startOpen={true}
                    >
                      {!currentHistoryJourney?.flashcardDecks ||
                      Object.keys(currentHistoryJourney.flashcardDecks)
                        .length === 0 ? (
                        <p
                          className={`${largeTextBase} text-brand-black/70 dark:text-gray-400`}
                        >
                          No flashcard decks created for this learning journey
                          yet.
                        </p>
                      ) : (
                        <div className="space-y-2">
                          {Object.entries(
                            currentHistoryJourney.flashcardDecks,
                          ).map(([deckModuleId, deck]) => {
                            return (
                              <button
                                key={deckModuleId}
                                onClick={() => {
                                  const moduleData =
                                    currentCurriculumModulesMap.get(
                                      deckModuleId,
                                    )?.module;
                                  if (moduleData) {
                                    setActiveFlashcardModuleInfo({
                                    ...
                                    setError(
                                      `Could not find module data for flashcard deck: ${deckModuleId}`,
                                    );
                                  }
                                }}
                              ...
                              >
                                <span className="font-semibold text-md text-brand-blue dark:text-blue-300">
                                  {deck.moduleTitle}
                                </span>
                                <span className="text-xs block text-brand-black/70 dark:text-gray-400">
                ...
                {curriculumSubTab === "exam" && (
                  <div>
                    ...
                    {examViewMode === "taking" &&
                      currentExamQuestions &&
                      currentExamQuestions[currentExamQuestionIndex] &&
                      currentExamAttempt && (
                        <ExamTakingView
                          question={
                            currentExamQuestions[currentExamQuestionIndex]
                          ...
                          timeLimitEnabled={
                            currentExamAttempt.config.timeLimitEnabled
                          }
                          examModuleTitle={
                            examModuleTitleForTakingDisplay || "Exam"
                          }
                        />
                      )}
                    {(reviewingExam ||
                      (examViewMode === "results" && examAttemptCompleted)) &&
                    examToShowInResultView ? (
                      <ExamResultsView
                        examAttempt={examToShowInResultView}
                        onRetake={() =>
                          handleStartRetakeExamFromHistory(
                            examToShowInResultView,
                          )
                        }
                        ...
                        onLoadDetailedExplanation={(qId) =>
                          handleLoadDetailedExplanation(qId, true)
                        }
                      />
                    ) : (
                      examViewMode === "module_selection" &&
                      !isGeneratingExam &&
                      !activeExamModuleInfo &&
                      !reviewingExam &&
                      !error && (
                        <div className="text-center p-6 bg-brand-lightGray dark:bg-gray-800 rounded-lg shadow">
                          ...
                          <h3
                            className={`${heading2Size} font-semibold text-brand-blue dark:text-blue-300 mb-3`}
                          >
                            Create or Configure an Exam
                          </h3>
                          <p
                            className={`${largeTextBase} text-brand-black/80 dark:text-gray-300 mb-4`}
                          >
                            To start a new exam, please select a module from the{" "}
                            <button
                              onClick={() => setCurriculumSubTab("material")}
                              className="text-brand-orange dark:text-orange-400 underline hover:text-brand-red dark:hover:text-red-500"
                            >
                            ...
                            tab and click "Create Exam", or select an exam from{" "}
                            <button
                              onClick={() => setCurriculumSubTab("study_log")}
                              className="text-brand-orange dark:text-orange-400 underline hover:text-brand-red dark:hover:text-red-500"
                            >
                          ...
                          <p
                            className={`${largeTextBase} text-brand-black/70 dark:text-gray-400`}
                          >
                            Ensure the module material is loaded before creating
                            an exam. If you were configuring an exam and landed
                            here, please re-select the module.
                          </p>
                        </div>
                      )
                    )}
                    {error && curriculumSubTab === "exam" && (
                      <p
                        className={`mt-4 text-brand-red dark:text-red-400 text-center`}
                      >
                        {error}
            ...
            {activeTab === "plan" && sevenDayPlan && (
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center pb-2 border-b border-brand-mediumGray dark:border-gray-700">
                  <h3
                    className={`${heading2Size} font-semibold text-brand-blue dark:text-blue-300 mb-1 sm:mb-0`}
                  >
                    7-Day Accelerated Learning Plan
                  </h3>
                  <span className="text-xs md:text-sm font-medium text-brand-orange dark:text-orange-400 bg-brand-orange/10 dark:bg-orange-950/40 px-3 py-1 rounded-full w-max">
                    7 Modul Terjadwal
                  </span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                  {sevenDayPlan.days.map((day: DailyPlan) => {
                    ...
                    const subtaskCompletion =
                      currentHistoryJourney?.planSubtaskCompletionStatus || {};

                    const completedSubtasksCount = subtasks.reduce(
                      (acc, _, idx) => {
                        const key = `${day.day}_${idx}`;
                        const isChecked =
                          key in subtaskCompletion
                            ? subtaskCompletion[key]
                            : currentHistoryJourney?.planTaskCompletionStatus[
                                day.day
                              ] || false;
                    ...
                    const cleanedFocusTitle = cleanModuleTitle(
                      day.summaryFocus,
                    );
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-2 pb-2 border-b border-brand-mediumGray/60 dark:border-gray-700/60 flex-wrap gap-1.5">
                            <div className="flex items-center space-x-2">
                              <span className="text-xs font-bold px-2 py-0.5 rounded bg-brand-blue text-brand-white">
                                Hari {day.day}
                              </span>
                              {totalSubtasksCount > 0 && (
                            ...
                            >
                              {isCompleted ? (
                                <>
                                  <Icons.CheckCircle className="w-3.5 h-3.5 inline mr-1" />
                                  Selesai
                                </>
                              ) : (
                                "Tandai Selesai"
                          ...
                          <h4
                            className="font-semibold text-sm md:text-base text-brand-blue dark:text-blue-300 mb-2 line-clamp-2 leading-snug break-words"
                            title={cleanedFocusTitle}
                          >
                            {cleanedFocusTitle}
                          </h4>
                          {subtasks.length > 0 ? (
                              ...
                              {subtasks.map((subtaskText, sIdx) => {
                                ...
                                const isSubtaskChecked =
                                  key in subtaskCompletion
                                    ? subtaskCompletion[key]
                                    : currentHistoryJourney
                                        ?.planTaskCompletionStatus[day.day] ||
                                      false;
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    />
                                    <div
                                      className={`flex-1 select-none leading-snug min-w-0 ${isSubtaskChecked ? "line-through opacity-70" : ""}`}
                                    >
                                      <MemoizedMarkdownRenderer
                                        content={subtaskText}
                                        baseTextSize="text-xs md:text-sm"
                                        inline
                                      />
                                    </div>
                                  </label>
                            ...
                            <div className="text-xs md:text-sm text-brand-black/80 dark:text-gray-300">
                              <MemoizedMarkdownRenderer
                                content={formatSevenDayTask(day.task)}
                                baseTextSize="text-xs md:text-sm"
                              />
                            </div>
                          )}
            ...
            {activeTab === "resources" && (
              <div className="p-4 md:p-6 bg-brand-white dark:bg-brand-black rounded-lg shadow-lg border border-brand-mediumGray dark:border-gray-700">
                <h3
                  className={`${heading2Size} font-semibold text-brand-blue dark:text-blue-300 mb-4`}
                >
                ...
                {!isFetchingResources &&
                  ...
                  !currentLearningResources && (
                    <div className="text-center py-8">
                      <p
                        className={`${largeTextBase} text-brand-black/70 dark:text-gray-400 mb-4`}
                      >
                        No resources loaded yet for this topic.
                      </p>
                      <button
                        onClick={handleFetchResources}
                ...
                {currentLearningResources && (
                  <>
                    <MemoizedMarkdownRenderer
                      content={currentLearningResources.content}
                      baseTextSize={largeTextBase}
                    />

                    {currentLearningResources.sources &&
                      currentLearningResources.sources.length > 0 && (
                        <div className="mt-6 pt-4 border-t border-brand-mediumGray dark:border-gray-600">
                          ...
                          <ul className="list-disc list-inside space-y-2">
                            {currentLearningResources.sources.map(
                              (source, index) => (
            ...
            {activeTab === "tutor" && (
              <div className="flex flex-col h-[calc(100vh-280px)] sm:h-[calc(100vh-260px)] md:h-[calc(100vh-240px)] bg-brand-white dark:bg-brand-black rounded-lg shadow-xl overflow-hidden border border-brand-mediumGray dark:border-gray-700">
                <div
                  id="chat-output"
                  className="flex-grow p-2 sm:p-4 space-y-3 md:space-y-4 overflow-y-auto scroll-smooth bg-brand-lightGray/50 dark:bg-gray-700/50 scrollbar-thin scrollbar-thumb-brand-mediumGray dark:scrollbar-thumb-gray-600 scrollbar-track-transparent"
                >
                  {tutorSession.history.map((msg) => (
                    ...
                    >
                      <div
                        className={`max-w-lg md:max-w-xl p-2.5 md:p-3 rounded-lg shadow flex items-end ${
                          msg.sender === "ai"
                      ...
                      >
                        <div
                          className={`whitespace-pre-wrap flex-grow text-sm sm:text-base md:text-lg`}
                        >
                          {msg.text === "..." ? (
                            <span className="italic">Tutor is typing...</span>
                          ) : (
                            <MemoizedMarkdownRenderer
                              content={msg.text}
                              baseTextSize="text-sm sm:text-base md:text-lg"
                            />
                          )}
                        </div>
                      </div>
                  ...
                  {isTutorTyping &&
                    tutorSession.history.length > 0 &&
                    tutorSession.history[tutorSession.history.length - 1]
                      .sender !== "ai" && (
                      <div className="flex justify-start">
                        <div className="max-w-lg md:max-w-xl p-2.5 md:p-3 rounded-lg shadow bg-brand-lightGray text-brand-black border border-brand-blue dark:bg-brand-blue dark:text-brand-white dark:border-transparent">
                          <p
                            className={`italic text-sm sm:text-base md:text-lg`}
                          >
                            Tutor is typing...
                          </p>
                        </div>
  ...
  return (
    <div className="min-h-screen md:h-screen md:w-screen md:overflow-hidden bg-brand-white text-brand-black dark:bg-brand-black dark:text-gray-100 flex flex-col md:flex-row transition-colors duration-300">
      ...
      <main className="flex-grow md:h-full md:overflow-y-auto transition-all duration-300 ease-in-out">
        ...
        <div
          className={`p-2 sm:p-3 md:p-4 lg:p-6 ${isSidebarVisible && typeof window !== "undefined" && window.innerWidth >= 768 ? "md:ml-0" : "md:ml-0"}`}
        >
